...
def db_update():
    ...
    sql = "UPDATE offerusers.productall SET decision=1 WHERE (decision=9)"
...
df_os = pd.read_sql('SELECT * FROM offerusers.productall where decision=1', con=connection)
df_new = pd.read_sql('SELECT * FROM offerusers.productall where decision=9', con=connection)
...
print("New prediction: ", model_bg.predict(X_new))
print("New prediction: ", model_xgb.predict(X_new))
print("New prediction: ", model_evc.predict(X_new))

if bagging_score == 1 & model_bg.predict(X_new) == 1:
    print("Offer Request Approved by Bagging model")
    db_update()
elif boosting_score == 1 & model_xgb.predict(X_new) == 1:
    print("Offer Request Approved by Boosting model")
    db_update()
elif voting_score == 1 & model_evc.predict(X_new) == 1:
